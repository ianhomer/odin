...
test('Sequences container renders OK', done => {
  ...
  expect(row.find('span.notation').at(0).text()).toBe('C A C5/h C5/8')
  row.simulate('click')
  expect(row).toMatchSnapshot()
  row.find('input.notation').simulate('change', {target: {value: 'C D E'}})
  expect(row).toMatchSnapshot()
  row.find('input.notation').simulate('keypress', {key: 'Enter'})
  expect(row).toMatchSnapshot()
  expect(row.find('span.notation').at(0).text()).toBe('C D E')
}, TIMEOUT)