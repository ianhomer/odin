...
test('Sequences container renders OK', done => {
  ...
  row.simulate('click')
  expect(row).toMatchSnapshot()
  row.find('input.property-notation').simulate('change', {target: {value: 'C D E'}})
  ...
  expect(row).toMatchSnapshot()
  expect(row.find('.property-notation').text()).toBe('C D E')
  //expect(row.find('.property-name').text()).toBe('aahs-changed')
  //expect(row.find('.property-channel').text()).toBe('5')
}, TIMEOUT)